...
while True:
    ...
    # Find the nearest person based on the largest height (h)
    ...
    # Draw bounding boxes, highlighting the nearest person
    for i in indices.flatten():
        x, y, w, h = boxes[i]
        if w * h < 5000:
            continue

        color = (0, 0, 255) if i == nearest_index else (0, 255, 0)  # Red for nearest, Green for others
        cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
        text = f"{class_names[class_ids[i]]}: {confidences[i]:.2f}"
        cv2.putText(frame, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

        # Display x-coordinate of the nearest person
        if i == nearest_index:
            print(f"Nearest person x-coordinate: {x}")
